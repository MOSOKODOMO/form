...
function renderRFQ(){
  ...
  $('#local-label').textContent=t('M2 · BROWSER PROTOTYPE','M2 · 浏览器原型');
  $('#intro').replaceChildren(el('p','FABRICATION INTELLIGENCE','eyebrow'),el('h1',t('A clearer brief. A better starting point.','清晰的需求，更好的起点。')),el('p',t('Describe your project for our personally managed sourcing service. Start with the product, dimensions and delivery needs.','描述项目所需的产品、尺寸及交付要求，为人工采购协调服务做好准备。'),'intro-copy'),el('p',t('This preview saves requests only in this browser. FI and fabricators cannot see them. Clearing browser data removes them; another device will not have them.','此预览仅在当前浏览器保存需求。FI 和制造商无法查看。清除浏览器数据将删除需求，其他设备无法访问。'),'notice'));
  $('#footer').textContent=t('Fabrication Intelligence · RFQ preview · Saved locally, not submitted.','Fabrication Intelligence · 询价预览 · 仅本地保存，未提交。');
  const screen=$('#screen');
  if(view==='rfqs'){
    screen.append(el('h2',t('My RFQs','我的询价需求')),el('p',t('Requests saved on this browser. No requests have been sent.','保存在此浏览器的需求。所有需求均未发送。'),'subheading'));
  ...
  if(rfqMode!=='edit'){
    ...
    screen.append(el('p',t('Fabricator selection awaits the checked directory. This request has no recipients and will not be sent.','制造商选择功能有待接入已核实目录。此需求没有收件人，不会发送。'),'notice'));
    ...
    else actions.append(rfqButton('Download brief (.txt)','下载需求（.txt）',downloadRFQ),rfqButton('My RFQs','我的询价需求',()=>{view='rfqs';render(true);},true));screen.append(actions);return;
  }
  ...
  form.append(grid,el('p',t('Fabricators: checked directory connection pending. You can prepare and save your brief now.','制造商：等待接入已核实目录。现在可填写并保存需求。'),'notice'));
