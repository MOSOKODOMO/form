'use strict';
// Stage 1 request data lives only in memory. No network calls or persistence.
...
function renderShell(){
  ...
  $('#local-label').textContent=t('STAGE 1 · LOCAL TEST','第一阶段 · 本地测试');
  ...
  const figure=el('figure',undefined,'aside-image'),img=el('img');img.src='assets/staircase.png';img.alt=t('AI concept of a steel and timber staircase','钢木楼梯的 AI 概念效果图');figure.append(img,el('figcaption',t('Concept image · Not a completed project','概念效果图 · 非已交付项目')));intro.append(figure,el('p',t('Local prototype. Nothing is sent to us or suppliers. Your entries disappear when this page is reloaded. Savings and supply are not guaranteed.','本地原型。内容不会发送给我们或供应商；刷新页面后填写内容将消失。不保证节省费用或供货。'),'notice'));
  ...
  $('#footer').textContent=t('Fabrication Intelligence · Stage 1 prototype · No orders, payments or supplier contact.','Fabrication Intelligence · 第一阶段原型 · 不创建订单、不收款、不联系供应商。');
}
function renderForm(){
  const screen=$('#screen');screen.append(el('h2',t('Tell us about your project','告诉我们你的项目需求')),el('p',t('Start with what you know. Only your name, suburb and postcode are required.','从已知信息开始。仅姓名、所在区及邮编为必填项。'),'subheading'));
  ...
  for(const g of groups){const fs=el('fieldset');fs.append(el('legend',t(...g.title)));const grid=el('div',undefined,'fields');for(const f of g.fields){const label=el('label',undefined,'field'+(f.wide?' wide':''));label.htmlFor=f.id;label.append(el('span',t(...f.label)+(f.required?' *':'')));let input;
    ...
    input.id=f.id;input.name=f.id;if(f.required)input.required=true;if(f.id==='postcode')input.inputMode='numeric';if(f.id==='name')input.autocomplete='name';if(values[f.id]!==undefined)input.value=values[f.id];
  ...
  const actions=el('div',undefined,'actions'),review=el('button',t('Review my request →','核对我的需求 →'),'primary');review.type='submit';actions.append(review);form.append(actions,el('p',t('Next: review your information before confirming the local test.','下一步：核对信息，然后确认本地测试。'),'saved-note'));
...
function loadSample(){values={name:'Alex (sample)',suburb:'Richmond',postcode:'3121',stage:'plans',style:'open',material:'steel-timber',height:'2800',width:'1000',length:'4000',timing:'6months',installation:'install',notes:t('SAMPLE ONLY: indoor straight staircase, oak-look treads and dark steel frame. Measurements are unverified.','仅为示例：室内直梯，橡木色踏步、深色钢结构，尺寸未经核实。'),quote:'18000',gst:'included',scope:t('FICTIONAL QUOTE: staircase, delivery and installation. Engineering not specified. Sample data, not market pricing.','虚构报价：楼梯、运输和安装，未说明工程设计费用。仅为示例数据，并非市场价格。')};errors={};render(true);}
...
function renderReview(){const screen=$('#screen');screen.append(el('h2',t('Review your request','核对你的需求')),el('p',t('Check the details below. You can still go back and change anything.','请核对下方信息。你仍可返回修改任何内容。'),'subheading'));for(const g of groups){const section=el('section',undefined,'review-group');section.append(el('h3',t(...g.title)));const dl=el('dl');for(const f of g.fields){const row=el('div');row.append(el('dt',t(...f.label)),el('dd',displayValue(f)));dl.append(row);}section.append(dl);screen.append(section);}screen.append(el('p',t('These are preliminary requirements. No pricing, engineering assessment or manufacturer availability has been confirmed.','以上仅为初步需求，尚未确认价格、工程评估或制造商供货情况。'),'notice'));const actions=el('div',undefined,'actions'),edit=el('button',t('← Edit details','← 修改信息'),'secondary'),confirm=el('button',t('Confirm local test','确认本地测试'),'primary');edit.onclick=()=>{step=0;render(true);};confirm.onclick=()=>{step=2;render(true);};actions.append(edit,confirm);screen.append(actions);}
function renderConfirmation(){const screen=$('#screen');screen.append(el('div','✓','success-icon'),el('h2',t('Your test request is complete','你的测试需求已完成')),el('p',t('Your information has been reviewed in this local prototype. Nothing has been submitted to a business or manufacturer.','你已在本地原型中核对了信息，未向任何企业或制造商提交内容。'),'subheading'));const next=el('div',undefined,'next');next.append(el('h3',t('What would happen in the real service?','正式服务中，接下来会发生什么？')));const list=el('ol');[t('We would clarify missing details with you.','我们会与你确认缺失的需求信息。'),t('We would approach suitable manufacturers for quotes.','我们会向适合的制造商询价。'),t('You would review comparable costs and decide whether to proceed.','你会查看同等范围的成本比较，再决定是否继续。')].forEach(s=>list.append(el('li',s)));next.append(list);screen.append(next,el('p',t('None of these steps has started. No request is saved after a page reload.','以上步骤均未开始。刷新页面后，本次需求不会保留。'),'notice'));const actions=el('div',undefined,'actions'),review=el('button',t('View my request','查看我的需求'),'secondary'),restart=el('button',t('Start another test','开始新的测试'),'primary');review.onclick=()=>{step=1;render(true);};restart.onclick=()=>{values={};errors={};step=0;render(true);};actions.append(review,restart);screen.append(actions);}
