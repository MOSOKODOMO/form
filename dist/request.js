...
function renderForm(){
  ...
  const actions=el('div',undefined,'actions'),review=el('button',t('Review my request →','核对我的需求 →'),'primary');review.type='submit';actions.append(review);form.append(actions,el('p',t('Next: review your information, then email it to us.','下一步：核对信息，然后通过邮件发送给我们。'),'saved-note'));
...
function renderReview(){const screen=$('#screen');screen.append(el('h2',t('Review your request','核对你的需求')),el('p',t('Check the details below. You can still go back and change anything.','请核对下方信息。你仍可返回修改任何内容。'),'subheading'));for(const g of groups){const section=el('section',undefined,'review-group');section.append(el('h3',t(...g.title)));const dl=el('dl');for(const f of g.fields){const row=el('div');row.append(el('dt',t(...f.label)),el('dd',displayValue(f)));dl.append(row);}section.append(dl);screen.append(section);}screen.append(el('p',t('These are preliminary requirements. No pricing, engineering assessment or manufacturer availability has been confirmed. The button below opens your email app with these details addressed to '+FI_CONTACT_EMAIL+'. Nothing is sent until you press send there.','以上仅为初步需求，尚未确认价格、工程评估或制造商供货情况。点击下方按钮将打开你的邮件应用，并将以上信息发送至 '+FI_CONTACT_EMAIL+'。在邮件应用中点击发送后，我们才会收到。'),'notice'));const actions=el('div',undefined,'actions'),edit=el('button',t('← Edit details','← 修改信息'),'secondary'),send=emailLink(t('Email this request to us','通过邮件发送需求'),stairRequestSubject(),stairRequestText(),()=>{step=2;render(true);});edit.type='button';edit.onclick=()=>{step=0;render(true);};actions.append(edit,send);screen.append(actions);}
function renderConfirmation(){const screen=$('#screen');screen.append(el('div','✓','success-icon'),el('h2',t('Now press send in your email app','请在邮件应用中点击发送')),el('p',t('Your email app should have opened with your request addressed to '+FI_CONTACT_EMAIL+'. We only receive it once you send that email.','你的邮件应用应已打开，需求已填写并发送至 '+FI_CONTACT_EMAIL+'。你发送邮件后我们才会收到。'),'subheading'));const next=el('div',undefined,'next');next.append(el('h3',t('What happens next?','接下来会发生什么？')));const list=el('ol');[t('We reply to clarify any missing details.','我们会回复你，确认缺失的需求信息。'),t('We approach suitable manufacturers for quotes.','我们会向适合的制造商询价。'),t('You review comparable costs and decide whether to proceed.','你会查看同等范围的成本比较，再决定是否继续。')].forEach(s=>list.append(el('li',s)));next.append(list);screen.append(next,el('p',t('Email app didn’t open? Email '+FI_CONTACT_EMAIL+' with your details. This page doesn’t keep your request after a reload.','邮件应用没有打开？请直接发送邮件至 '+FI_CONTACT_EMAIL+' 并附上需求信息。刷新页面后本页不会保留你的需求。'),'notice'));const actions=el('div',undefined,'actions'),review=el('button',t('View my request','查看我的需求'),'secondary'),again=emailLink(t('Open email again','重新打开邮件'),stairRequestSubject(),stairRequestText()),restart=el('button',t('Start another request','新建需求'),'secondary');again.className='secondary button-link';review.type='button';restart.type='button';review.onclick=()=>{step=1;render(true);};restart.onclick=()=>{values={};errors={};step=0;render(true);};actions.append(review,again,restart);screen.append(actions);}
